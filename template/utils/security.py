...
def validate_init_script(command_string: str) -> tuple[bool, str]:
    ...
    # Only allow explicitly safe invocations (no path traversal)
    # Case 1: ./init.sh [args...]
    if tokens[0] == "./init.sh":
        return True, ""

    # Case 2: bash init.sh [args...] or sh init.sh [args...]
    if tokens[0] in ("bash", "sh") and len(tokens) > 1 and tokens[1] == "init.sh":
        return True, ""

    return False, f"Only ./init.sh, bash init.sh, or sh init.sh allowed, got: {command_string}"
